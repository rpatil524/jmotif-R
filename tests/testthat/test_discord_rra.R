...
test_that("RRA tier-B exact span w100 seed=0 (rra_ecg_top_region)", {
  ecg <- scan(system.file("test_data/ecg0606_1.csv", package = "jmotif"), sep = ",", quiet = TRUE)
  discords <- find_discords_rra(ecg, 100, 4, 4, "none", 0.01, 1, seed = 0L)
...
test_that("RRA tier-B exact span w120 seed=0 (rra_ecg_top_region_w120)", {
  ecg <- scan(system.file("test_data/ecg0606_1.csv", package = "jmotif"), sep = ",", quiet = TRUE)
  discords <- find_discords_rra(ecg, 120, 4, 4, "none", 0.01, 1, seed = 0L)
  expect_equal(discords[1, ]$start, 430L)
...
test_that("RRA w150/p7/a4 does not emit a zero-gap top discord", {
  ecg <- scan(system.file("test_data/ecg0606_1.csv", package = "jmotif"), sep = ",", quiet = TRUE)
  discords <- find_discords_rra(ecg, 150, 7, 4, "none", 0.01, 1, seed = 0L)
